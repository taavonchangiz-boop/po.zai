...
export async function requestOtp(mobileRaw: string, purpose: "register" | "reset"): Promise<{ sent: boolean; cooldownSec: number; errorFa?: string }> {
  ...
  if (!rl.ok) return { sent: false, cooldownSec: 600, errorFa: "تعداد درخواست کد بیش از حد مجاز است. ۱۵ دقیقه بعد تلاش کنید." };
  ...
  if (purpose === "reset" && !existing) return { sent: false, cooldownSec: 0, errorFa: "حسابی با این شماره یافت نشد." };
...
export async function verifyOtp(mobileRaw: string, codeRaw: string, purpose: "register" | "reset", ip?: string): Promise<{ ok: boolean; userId?: string; errorFa?: string }> {
  const mobile = normalizeMobile(mobileRaw);
